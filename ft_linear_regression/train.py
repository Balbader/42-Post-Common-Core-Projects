...
def train_model(mileages, prices, learning_rate=0.01, epochs=1000):
    ...
    for epoch in range(epochs):
        ...
        predictions = [estimate_price(mileage, theta0, theta1, mileage_mean,
                                      mileage_std) for mileage in mileages]

        # Calculate temporary theta values
        tmp_theta0 = (learning_rate * (1/m) *
                      sum(predictions[i] - normalized_prices[i]
                          for i in range(m)))
        ...
        if epoch % 100 == 0:
            print(f"Epoch {epoch}: theta0 =\
                  {theta0:.4f}, theta1 = {theta1:.4f}")
...
def main():
    ...
    try:
        ...
        print("\nNote: Install matplotlib and scikit-learn\
              to see visualization and metrics")
